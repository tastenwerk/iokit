      ...
      if( req.url.replace('/files/','').indexOf('/') < 0 )
        File.findById( konter.mongoose.Types.ObjectId( req.params.id ) ).execWithUser( user, function( err, file ){
          ...
          if( file ){
            ...
            res.sendfile( path.join( inter.config.datastore.absolutePath, 'files', file._id.toString().substr(0,2), file._id.toString(), 'orig' ) );
          }
          else
...
function findFileByNameAndAncestors( user, req, res ){
  ...
  File.find({ name: paths[paths.length-1] }).execWithUser( user, function( err, files ){
    ...
    if( files.length === 1 ){
      ...
      res.sendfile( path.join( inter.config.datastore.absolutePath, 'files', files[0]._id.toString().substr(0,2), files[0]._id.toString(), 'orig' ) );
    } else
          ...
          for( var i=0,file; file=files[i]; i++ )
            for( var j=0,filePath; filePath=file.paths[j]; j++ )
              if( filePath.indexOf( doc._id.toString() ) >= 0 ){
                res.setHeader('Content-Type', files[0].contentType);
                return res.sendfile( path.join( inter.config.datastore.absolutePath, 'files', file._id.toString().substr(0,2), file._id.toString(), 'orig' ) );
              }
          res.send(404);
        } else
          res.send(404);
