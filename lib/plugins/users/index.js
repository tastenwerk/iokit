...
inter.view.paths = [ __dirname + '/views' ].concat( inter.view.paths );
...
module.exports = exports = {
  ...
  routes: function( app ){
    ...
    app.put('/users/:id/change_pic_modal', auth.check, getUser, function( req, res ){
      ...
      console.log('coords for changing user pic', req.body);
    ...
    app.get('/users.json', auth.check, function( req, res ){
      var q = User.find();
      if( req.query.online )
        q.where('lastRequest.createdAt').gte( moment().subtract('m', inter.config.session.timeout.mins ) );
    ...
    app.get('/users/:id', auth.check, getUser, function( req, res ){
      ...
      res.render( inter.view.lookup('users/show.ejs'), {user: req.user, partialPath: __dirname + '/views/users/show.jade'} );
    });
    ...
    app.put('/users/:id', auth.check, getUser, function( req, res ){
      ...
      if( req.user ){
        userAttrs = { name: {
            nick: req.body.user.name.nick,
            first: req.body.user.name.first,
            last: req.body.user.name.last
          }
        };
        if( res.locals.currentUser.roles.indexOf('manager') >= 0 )
          userAttrs.roles = req.body.user.roles;
        console.log(userAttrs);
        req.user.update( userAttrs, function( err ){
          if( err ){
            console.log(err);
...
function resizeAndCopyImage( origName, userDirPath, callback ){
  ...
  easyimg.resize( resizeOpts, function( err, image ){
    console.log( err )
    console.log( image );
    console.log('copy from profile.jpg to _150');
