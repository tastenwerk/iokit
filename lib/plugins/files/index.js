...
var filesPlugin = {
  ...
  routes: function( app ){
    ...
    app.get('/files/:id', auth.check, function( req, res ){
      File.findById( konter.mongoose.Types.ObjectId( req.params.id ) ).execWithUser( res.locals.currentUser, function( err, file ){
        if( err ) return res.send(404);
        res.sendfile( path.join( inter.config.datastore.absolutePath, file.relativePath, 'orig'+path.extname(file.name) ) );
      })
    });
